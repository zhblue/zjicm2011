...
<?php
templates("");
$templateHead = str_replace("{title}",'游戏-'.$webname,$templateHead);
...
<?php
//游戏列表
$query = 'select * from video where vtype="games" order by vid desc limit '.$voffset.',4';
...
if($_GET['url']){
?>
<a href="game.php" style="text-align:left;text-decoration:underline;"><h3 style="color: #FF0000; font-size: 20px;">游戏列表</h3>
</a><br/>
...
<?php
if ($vpage != 1) echo '<a href="game.php">首页</a> ';
if ($vpage != 1) echo '<a href="game.php?p='.($vpage-1).'">上一页</a> ';
for ($i=1;$i<$vpagenum+1;$i++) {
	if ($page == $i) echo '<strong>'.$i.'</strong> ';
	else echo '<a href="game.php?p='.$i.'">'.$i.'</a> ';
}
if ($vpage != $vpagenum) echo '<a href="game.php?p='.($vpage+1).'">下一页</a>';
if ($vpage != $vpagenum) echo '<a href="game.php?p='.$vpagenum.'">尾页</a>';
}?>
			</div>
			</div>
...
echo '很抱歉，只有登录才能查看游戏！请<a href="'.$Dir.'index.php" color="#FF0000" style="color:#FF0000;">登录</a>';
}
