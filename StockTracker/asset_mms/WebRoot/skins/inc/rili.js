...
strFrame+='<tr Author="wayx" valign="middle" align="center"><td style="font-size:12px;color:#FFFFFF" Author=webjx><b>日</b></td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" Author=webjx><b>一</b></td><td style="font-size:12px;color:#FFFFFF" Author=webjx><b>二</b></td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" Author=webjx><b>三</b></td><td style="font-size:12px;color:#FFFFFF" Author=webjx><b>四</b></td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" Author=webjx><b>五</b></td><td style="font-size:12px;color:#FFFFFF" Author=webjx><b>六</b></td></tr>';
strFrame+='</table></td></tr><!-- Author:F.R.Huang(webjx) http://www.dwww.cn/ mail: webjx@hzcnc.com 2002-10-8 -->';
...
strFrame+='<td width=25 height=25 id=webjxDay'+n+' style="font-size:12px" Author=webjx onclick=parent.webjxDayClick(this.innerText,0)></td>';n++;}
strFrame+='</tr>';}
strFrame+=' <tr align=center Author="wayx">';
for (i=35;i<39;i++)strFrame+='<td width=25 height=25 id=webjxDay'+i+' style="font-size:12px" Author=wayx onclick="parent.webjxDayClick(this.innerText,0)"></td>';
strFrame+=' <td colspan=3 align=right Author=webjx><span onclick=parent.closeLayer() style="font-size:12px;cursor: hand"';
strFrame+=' Author=webjx title="' + _VersionInfo + '"><u>关闭</u></span> </td></tr>';
strFrame+=' </table></td></tr><tr Author="wayx"><td Author="wayx">';
strFrame+=' <table border=0 cellspacing=1 cellpadding=0 width=100% Author="wayx" bgcolor=#FFFFFF>';
strFrame+=' <tr Author="wayx"><td Author=webjx align=left><input Author=webjx type=button class=button style="cursor:hand" value="<<" title="向前翻 1 年" onclick="parent.webjxPrevY()" ';
strFrame+=' onfocus="this.blur()" style="font-size: 12px; height: 20px"><input Author=webjx class=button title="向前翻 1 月" type=button ';
strFrame+=' value="< " style="cursor:hand" onclick="parent.webjxPrevM()" onfocus="this.blur()" style="font-size: 12px; height: 20px"></td><td ';
strFrame+=' Author=webjx align=center><input Author=webjx style="cursor:hand" type=button class=button value=Today onclick="parent.webjxToday()" ';
strFrame+=' onfocus="this.blur()" title="当前日期" style="font-size: 12px; height: 20px; cursor:hand"></td><td ';
strFrame+=' Author=webjx align=right><input Author=webjx type=button class=button value=" >" style="cursor:hand" onclick="parent.webjxNextM()" ';
strFrame+=' onfocus="this.blur()" title="向后翻 1 月" class=button style="font-size: 12px; height: 20px"><input ';
strFrame+=' Author=webjx type=button class=button style="cursor:hand" value=">>" title="向后翻 1 年" onclick="parent.webjxNextY()"';
strFrame+=' onfocus="this.blur()" style="font-size: 12px; height: 20px"></td>';
strFrame+='</tr></table></td></tr></table></td></tr></table></div>';

window.frames.webjxDateLayer.document.writeln(strFrame);
window.frames.webjxDateLayer.document.close(); //解决ie进度条不结束的问题

//==================================================== WEB 页面显示部分 ======================================================
var outObject;
var outButton; //点击的按钮
var outDate=""; //存放对象的日期
var odatelayer=window.frames.webjxDateLayer.document.all; //存放日历对象
...
var webjxTheYear=new Date().getFullYear(); //定义年的变量的初始值
var webjxTheMonth=new Date().getMonth()+1; //定义月的变量的初始值
var webjxWDay=new Array(39); //定义写日期的数组



function webjxWriteHead(yy,mm) //往 head 中写入当前的年与月
{
...
if (n < 1000) n = 1000;
if (n + 26 > 9999) n = 9974;
var s = " <select Author=webjx name=tmpSelectYear style='font-size: 12px' "
...
var s = " <select Author=webjx name=tmpSelectMonth style='font-size: 12px' "
...
function webjxPrevY() //往前翻 Year
{
if(webjxTheYear > 999 && webjxTheYear <10000){webjxTheYear--;}
else{alert("年份超出范围（1000-9999）！");}
webjxSetDay(webjxTheYear,webjxTheMonth);
}
function webjxNextY() //往后翻 Year
{
if(webjxTheYear > 999 && webjxTheYear <10000){webjxTheYear++;}
else{alert("年份超出范围（1000-9999）！");}
webjxSetDay(webjxTheYear,webjxTheMonth);
}
function webjxToday() //Today Button
{
var today;
...
function webjxPrevM() //往前翻月份
{
if(webjxTheMonth>1){webjxTheMonth--}else{webjxTheYear--;webjxTheMonth=12;}
webjxSetDay(webjxTheYear,webjxTheMonth);
}
function webjxNextM() //往后翻月份
{
if(webjxTheMonth==12){webjxTheYear++;webjxTheMonth=1}else{webjxTheMonth++}
webjxSetDay(webjxTheYear,webjxTheMonth);
}

function webjxSetDay(yy,mm) //主要的写程序**********
{
...
for (var i = 0; i < 39; i++){webjxWDay[i]=""}; //将显示框的内容全部清空
var day1 = 1,day2=1,firstday = new Date(yy,mm-1,1).getDay(); //某月第一天的星期几
for (i=0;i<firstday;i++)webjxWDay[i]=GetMonthCount(mm==1?yy-1:yy,mm==1?12:mm-1)-firstday+i+1 //上个月的最后几天
for (i = firstday; day1 < GetMonthCount(yy,mm)+1; i++){webjxWDay[i]=day1;day1++;}
for (i=firstday+GetMonthCount(yy,mm);i<39;i++){webjxWDay[i]=day2;day2++}
for (i = 0; i < 39; i++)
{ var da = eval("odatelayer.webjxDay"+i) //书写新的一个月的日期星期排列
if (webjxWDay[i]!="")
{ 
//初始化边框
...
da.onclick=Function("webjxDayClick(this.innerText,-1)");

if(!outDate)
da.style.backgroundColor = ((mm==1?yy-1:yy) == new Date().getFullYear() && 
(mm==1?12:mm-1) == new Date().getMonth()+1 && webjxWDay[i] == new Date().getDate()) ?
"#E4E3F2":"#FFFFFF";
else
{
da.style.backgroundColor =((mm==1?yy-1:yy)==outDate.getFullYear() && (mm==1?12:mm-1)== outDate.getMonth() + 1 && 
webjxWDay[i]==outDate.getDate())? "#E8F5E7" : // 选中日期颜色
(((mm==1?yy-1:yy) == new Date().getFullYear() && (mm==1?12:mm-1) == new Date().getMonth()+1 && 
webjxWDay[i] == new Date().getDate()) ? "#E4E3F2":"#FFFFFF"); // 当前系统时间颜色
//将选中的日期显示为凹下去
if((mm==1?yy-1:yy)==outDate.getFullYear() && (mm==1?12:mm-1)== outDate.getMonth() + 1 && 
webjxWDay[i]==outDate.getDate())
{
...
(mm==12?1:mm+1) == new Date().getMonth()+1 && webjxWDay[i] == new Date().getDate()) ?
"#E4E3F2":"#FFFFFF";
else
{
...
webjxWDay[i] == new Date().getDate()) ? "#E4E3F2":"#FFFFFF"); // 当前系统时间
//将选中的日期显示为凹下去
...
da.onclick=Function("webjxDayClick(this.innerText,0)"); //给td赋予onclick事件的处理
//如果是当前选择的日期，则显示亮蓝色的背景；如果是当前日期，则显示暗黄色背景
if(!outDate)
da.style.backgroundColor = (yy == new Date().getFullYear() && mm == new Date().getMonth()+1 && webjxWDay[i] == new Date().getDate())?
"#FFFFFF":"#FFFFFF";
else
{
da.style.backgroundColor =(yy==outDate.getFullYear() && mm== outDate.getMonth() + 1 && webjxWDay[i]==outDate.getDate())?
"#D5ECD2":((yy == new Date().getFullYear() && mm == new Date().getMonth()+1 && webjxWDay[i] == new Date().getDate())?
